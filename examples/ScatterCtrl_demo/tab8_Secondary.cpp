...
void Tab8_Secondary::Init()
{
	...
	double barWidth = 0.4;
	scatter.AddSeries(s3).Legend("Speed").Units("km/h").PlotStyle<BarSeriesPlot>().BarWidth(barWidth).NoMark().Stroke(2, Gray()).Dash(ScatterDraw::LINE_SOLID).Fill(LtGray());
	...
	scatter.AddSeries(s1).Legend("Temperature").Units("ºC").Stroke(3, Null).Dash(ScatterDraw::LINE_DASHED).MarkStyle<SquareMarkPlot>();
